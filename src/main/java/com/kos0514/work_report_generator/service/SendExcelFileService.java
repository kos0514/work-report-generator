...
public class SendExcelFileService {
    ...
    public String sendExcelFile(String fileName) {
        try {
            ...
            // ZIPファイル名の生成
            String zipFileName = fileName.replaceFirst("\\" + Constants.Files.EXCEL_EXTENSION + "$", Constants.Files.ZIP_EXTENSION);
            ...
            return "ファイルを送信しました:\n" +
                    ...
                    "- ZIP: " + zipFilePath + "\n" +
                    "- パスワード: " + password + "\n" +
                    "- パスワード保存先: " + Paths.get(sendDir, Constants.Files.WORK_DIR, yearMonth.substring(0, 4), yearMonth, Constants.Files.PASSWORD_FILE_NAME);
        } catch (Exception e) {
